...
- Обучение с нуля НЕ делается. «Самообучение» = память + периодический LoRA
  (см. раздел 9). Веса на лету не переписываются.
...
## 9. Самообучение (реалистично, без фантастики)

Не «переписывание весов на лету», а три механизма:
1. **Память/персона** — ассистент через persona.append_memory дописывает
   заметки о пользователе и стиле; они идут в system-prompt.
2. **Периодический LoRA** — оффлайн-скрипт (scripts/finetune_lora.md):
   выгрузка твоих исходящих сообщений из архива → датасет в формате
   инструкций → QLoRA 4-bit (unsloth) на базовой 7–8B → слияние/адаптер →
   импорт в Ollama через Modelfile. Запускается вручную, раз в N дней.
   На 8 GB VRAM реально для 7B в 4-bit, но медленно — это норма.
3. **Инструменты/плагины** (позже) — расширяемый список функций, которые
   модель может вызывать (поиск по архиву, факты и т.п.).
Всё это управляемо и обратимо. Литеральное самопереписывание кода
модели НЕ реализуем — на этом железе нереально и небезопасно.
...
7. run.py — всё вместе.
8. scripts/finetune_lora.md — по желанию, в конце.

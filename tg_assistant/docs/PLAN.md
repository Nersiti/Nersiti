...
Страницы/эндпоинты:
- `GET /` — список чатов. В КАЖДОЙ строке чата:
  - тумблер enabled, селект mode (off/draft/auto), при auto — селект
    submode (preset/generate/dialogue), поле preset_text;
  - КНОПКА «Архив чата» -> ссылка на `GET /chat/{id}/archive`.
- `POST /chat/{id}/settings` — сохранить настройки чата.
- `GET /chat/{id}/archive` — ПОЛНЫЙ архив переписки этого чата из локальной
  базы (это и есть «кнопка с архивом чата»). Показывает ВСЕ сохранённые
  сообщения по возрастанию даты:
  - входящие и исходящие (свои отправленные — тоже, они запомнены);
  - удалённые (`is_deleted=1`) — помечены визуально «удалено», но текст виден;
  - отредактированные — показываются версии (по edited_at), видно правки;
  - медиа — превью/ссылка на файл в `storage.data_dir/media/...`;
  - reply_to — на какое сообщение отвечали.
  Пагинация (напр. по 200), поиск внутри чата (?q=), быстрый переход
  к дате. Данные берём из storage.db (таблицы messages/media), НЕ из Telegram.
- `GET /drafts` — очередь черновиков (pending): текст, кнопки
  «Отправить», «Изменить+Отправить», «Отклонить».
- `POST /drafts/{id}/approve`, `/reject`.
- `GET /search?q=...` — полнотекстовый поиск по ВСЕМУ архиву (FTS5),
...
Для `/chat/{id}/archive` в storage/db.py нужен помощник
`get_chat_history(chat_id, limit, offset, q=None) -> list[Message]`
(с учётом версий правок и удалённых).
